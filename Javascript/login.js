...
// Creating a variable "existingUser" which pulls the string information from localStorage (server) and sets it to an object.
...
// Check if username and password match with already existing user. Redirect to main page if it returns true
    for (let i = 0; i < existingUser.length; i++) { // Runs through the localStorage via the exisitngUser variable to see if there is a match
       // console.log(usernameInput)
        ...
        if (usernameInput == existingUser[i].userName && passwordInput == existingUser[i].psw) {
            // If username and psw matches, Push username to current user in LocalStorage and redirect to home page.
